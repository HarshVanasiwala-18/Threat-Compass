    ...
    with open(os.path.join(new_folder_path,'indicators.csv'), 'w', newline='') as file:
        ...
        while True:
            try:
                ...
                response = requests.get(f'https://otx.alienvault.com/otxapi/indicators/?include_inactive=0&sort=-modified&q={Threat_Actor}&page={j}&limit=10000')
                ...
                if response.status_code == 200:
                    ...
                    if json_data['results'] != []:
                        for i in range(len(json_data['results'])):
                            writer.writerow([json_data['results'][i]['indicator'], json_data['results'][i]['type']])
...
def scarpe_pulses(Threat_Actor):
    ...
    with open(os.path.join(new_folder_path, 'pulses_id.csv'), 'w', newline='') as file:
        ...
        while True:
            try:
                Threat_Actor = Threat_Actor.replace(' ', '%20')
                response = requests.get(f'https://otx.alienvault.com/otxapi/pulses/?limit=10000&page={j}&sort=-modified&q={Threat_Actor}')
                soup = BeautifulSoup(response.content, 'html.parser')
                j += 1
                if response.status_code == 200:
                    json_data = json.loads(soup.prettify())
...
def scrape_ioc_pulses(Threat_Actor):
    ...
    with open(os.path.join(new_folder_path, 'pulses_id.csv'), 'r') as file:
        ...
        with open(os.path.join(new_folder_path, 'pulses_ioc.csv'), 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["Indicator", "Type"])
            for row in reader:
                ...
                while True:
                    try:
                        response = requests.get(f'https://otx.alienvault.com/otxapi/pulses/{row[0]}/indicators/?sort=-created&limit=10000&page={j}')
                        soup = BeautifulSoup(response.content, 'html.parser')
                        json_data = json.loads(soup.prettify())
