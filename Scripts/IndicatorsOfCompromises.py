...
def scarpe_indicators(Threat_Actor):
    ...
    with open(f"CTI DB/Indicators/{name}_direct_pulse.csv", "w", newline="") as file:
                ...
                if response.status_code == 200:
                    ...
                    if json_data["results"] != []:
                        for i in range(len(json_data["results"])):
                            ...
                            type = json_data["results"][i]["type"]
                            print("Scraping: ", indicator, " - ", type, end="", flush=True)
                            writer.writerow([indicator, type])
...
def scrape_ioc_pulses(Threat_Actor, pulses):
    name = Threat_Actor.replace(" ", "_")
    with open(f"CTI DB/Indicators/{name}_valid_multiple_pulse.csv", "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Indicator", "Type"])
        reader = pulses
        for row in reader:
                        ...
                        if json_data["results"] != []:
                            for i in range(len(json_data["results"])):
                                ...
                                if count >= 10:
                                    ...
                                    print("Scraping: ", indicator, " - ", ioc_type, " - ", count, end="", flush=True)
...
def main():
    try:
        ...
        directory = "CTI DB/Indicators"
        if os.path.exists(directory + "/" + user_input.replace(" ", "_") + "_valid_multiple_pulse.csv"):
            print("File already exists")
            print("Please rename or delete the file")
            sys.exit(1)
        elif os.path.exists(directory + "/" + user_input.replace(" ", "_") + "_direct_pulse.csv"):
            print("File already exists")
            print("Please rename or delete the file")
            sys.exit(1)
        else:
